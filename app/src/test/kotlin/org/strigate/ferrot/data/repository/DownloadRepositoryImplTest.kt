...
class DownloadRepositoryImplTest {
    ...
    fun save_insertsMappedEntity() = runTest(testDispatcher) {
        ...
        assertEquals(download.archived, insertedEntity?.archived)
    ...
    fun updateMethods_delegateToDao() = runTest(testDispatcher) {
        ...
        assertEquals(2, repository.updateSeenByIds(setOf(3L, 4L), false))
        assertEquals(2, repository.updatePendingDeleteByIds(setOf(3L, 4L), true))
        assertEquals(2, repository.updateArchivedByIds(setOf(3L, 4L), true))
